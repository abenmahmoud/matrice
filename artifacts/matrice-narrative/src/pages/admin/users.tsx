...
export default function AdminUsersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error("Liste utilisateurs inaccessible");
      return response.json() as Promise<UsersPayload>;
    },
  });

  return (
    <AdminShell title="Utilisateurs" subtitle="Recherche, filtres, support et suivi des comptes beta depuis une seule vue.">
      <section className="rounded-2xl border border-matrice-sable bg-white p-4 shadow-sm sm:p-5">
        <div className="grid gap-3 md:grid-cols-[1fr_180px_180px_auto]">
          ...
          <select className="rounded-md border border-matrice-sable bg-white px-3 text-sm text-matrice-encre focus:outline-none focus:ring-2 focus:ring-matrice-or-fonce" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }}>
            <option value="all">Tous les statuts</option>
            <option value="active">Actifs</option>
            <option value="suspended">Suspendus</option>
            <option value="deleted">Supprimes</option>
          </select>
          <AdminButton variant="secondary" onClick={() => { setSearch(""); setPlan("all"); setStatus("all"); setPage(1); }}>
            Reinitialiser
          </AdminButton>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-matrice-sable bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-matrice-sable px-4 py-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-matrice-encre">
            <Users className="h-4 w-4 text-matrice-or-fonce" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-sm">
            ...
            <tbody>
              {isLoading ? (
                <tr><td className="px-4 py-6 text-matrice-encre/65" colSpan={7}>Chargement...</td></tr>
                ...
                <tr key={user.id} className="border-b border-matrice-sable/70 odd:bg-white even:bg-matrice-sable/20">
                  ...
                  <td className="px-4 py-3 text-matrice-encre/75">{user.projectsCreated} projets / {user.generationsUsed} generations</td>
                  <td className="px-4 py-3 text-matrice-encre/75">{user.onboardingStep}</td>
                  <td className="px-4 py-3 text-matrice-encre/65">{new Date(user.createdAt).toLocaleDateString("fr-FR")}</td>
                ...
                <tr><td className="px-4 py-6 text-matrice-encre/65" colSpan={7}>Aucun utilisateur trouve.</td></tr>
              )}
            </tbody>
          </table>
        </div>
