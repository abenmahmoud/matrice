...
export function EditableField({ label, value, onSave, multiline, className }: EditableFieldProps) {
  ...
  return (
    <div className="space-y-1">
      {label ? <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{label}</p> : null}
      {editing ? (
        multiline ? (
          <Textarea
            autoFocus
        ...
        <p
          className={cn(
            "text-sm text-foreground/80 leading-relaxed cursor-text rounded-md px-2 py-1.5 hover:bg-white/5 transition-colors min-h-[2rem] whitespace-pre-wrap",
            className,
          )}
